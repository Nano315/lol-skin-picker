...
class Body {
  ...
  async formData() {
    ...
    const { toFormData } = await import("./multipart-parser-mGx7udYy.js");
...
function configureFromLockfile(raw) {
  const [, port, password, protocol] = raw.trim().split(":");
  lcuBase = `${protocol}://127.0.0.1:${port}`;
  const tok = Buffer.from(`riot:${password}`).toString("base64");
  lcuHeaders = { Authorization: `Basic ${tok}` };
}
function startGameflowWatch() {
  if (gfTimer) return;
  gfTimer = setInterval(async () => {
    try {
      const res = await fetch(`${lcuBase}/lol-gameflow/v1/gameflow-phase`, {
        agent: new https.Agent({ rejectUnauthorized: false }),
        headers: lcuHeaders
      });
      if (!res.ok) throw new Error("HTTP " + res.status);
      const phase = (await res.text()).replace(/"/g, "");
      if (phase !== gameflow) {
        gameflow = phase;
        win == null ? void 0 : win.webContents.send("gameflow", gameflow);
      }
    } catch {
    }
  }, 2e3);
}
function stopGameflowWatch() {
  if (gfTimer) {
...
function watchLcu() {
  setInterval(() => {
    ...
    const newStatus = raw ? "connected" : "disconnected";
    if (newStatus !== status) {
      status = newStatus;
      win == null ? void 0 : win.webContents.send("lcu-status", status);
      if (status === "connected" && raw) {
        configureFromLockfile(raw);
        startGameflowWatch();
      } else {
        stopGameflowWatch();
      }
...
ipcMain.handle("get-gameflow", () => gameflow);
