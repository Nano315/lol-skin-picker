...
class Body {
  ...
  async formData() {
    ...
    const { toFormData } = await import("./multipart-parser-DirHycwY.js");
...
class Picker extends EventEmitter {
  ...
  /* -------- skins + chromas -------- */
  async loadSkins(cid) {
    const summ = await this.lcu.get(
      "/lol-summoner/v1/current-summoner"
    );
    if (!summ) return;
    const skinsInv = await this.lcu.get(
      `/lol-champions/v1/inventories/${summ.summonerId}/champions/${cid}/skins`
    ) || [];
  ...
  async fetchChromas(cid, sid, summId) {
    var _a2;
    const chromasInv = await this.lcu.get(
      `/lol-champions/v1/inventories/${summId}/champions/${cid}/skins/${sid}/chromas`
    ) || [];
