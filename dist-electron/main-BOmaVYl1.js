...
class Body {
  ...
  async formData() {
    ...
    const { toFormData } = await import("./multipart-parser-2aLVqxvc.js");
...
const POLL_MS = 2e3;
const LOCKFILE = process.platform === "win32" ? "C:/Riot Games/League of Legends/lockfile" : "/Applications/League of Legends.app/Contents/LoL/lockfile";
const CDR_SUMMARY = "https://raw.communitydragon.org/latest/plugins/rcp-be-lol-game-data/global/default/v1/champion-summary.json";
const CDR_DETAIL = "https://raw.communitydragon.org/latest/plugins/rcp-be-lol-game-data/global/default/v1/champions/{cid}.json";
const champName = {};
const champDetailCache = {};
async function loadNames() {
  if (Object.keys(champName).length) return;
  try {
    const data = await (await fetch(CDR_SUMMARY)).json();
    data.forEach((c) => champName[c.id] = c.name);
  } catch {
  }
}
async function getDetails(cid) {
  if (!champDetailCache[cid]) {
    const url = CDR_DETAIL.replace("{cid}", String(cid));
    try {
      champDetailCache[cid] = await (await fetch(url)).json();
    } catch {
      champDetailCache[cid] = {};
    }
  }
  return champDetailCache[cid];
}
class LCU {
  constructor() {
  ...
  async connect() {
    try {
      ...
      const [port, pw, proto] = parts.length === 5 ? [parts[2], parts[3], parts[4]] : [parts[1], parts[2], parts[3]];
      this.base = `${proto}://127.0.0.1:${port}`;
      const token = Buffer.from(`riot:${pw}`).toString("base64");
      this.headers = { Authorization: `Basic ${token}`, "Content-Type": "application/json" };
      this.agent = proto === "https" ? new https.Agent({ rejectUnauthorized: false }) : void 0;
      this.active = true;
      return true;
    } catch {
      this.active = false;
  ...
  async get(ep) {
    ...
    try {
      const init = { headers: this.headers, agent: this.agent, timeout: 4e3 };
      const r2 = await fetch(this.base + ep, init);
      return r2.ok ? r2.text() ? await r2.json() : null : null;
    } catch {
      return null;
    }
  }
...
class Picker extends EventEmitter {
  constructor() {
    super(...arguments);
    __publicField(this, "lcu", new LCU());
    __publicField(this, "state", { connected: false, phase: "Unknown", championId: null, championName: null, skins: [] });
  }
  start() {
  ...
  async tick() {
    const ok = await this.lcu.connect();
    if (ok !== this.state.connected) {
      this.state = { connected: ok, phase: "Unknown", championId: null, championName: null, skins: [] };
    ...
    const cid = await this.lcu.get("/lol-champ-select/v1/current-champion");
    if (!cid || cid === 0) return;
    if (cid !== this.state.championId) {
      await loadNames();
      this.state.championId = cid;
      this.state.championName = champName[cid] || `#${cid}`;
      await this.loadSkins(cid);
      this.emit("update", this.state);
    }
  }
  async loadSkins(cid) {
    const summ = await this.lcu.get("/lol-summoner/v1/current-summoner");
    if (!summ) return;
    const skins = await this.lcu.get(`/lol-champions/v1/inventories/${summ.summonerId}/champions/${cid}/skins`) || [];
    const owned = skins.filter(this.isOwned.bind(this));
    this.state.skins = [];
    for (const s of owned) {
      const chromas = await this.loadChromas(cid, s.id, summ.summonerId);
      this.state.skins.push({ id: s.id, name: s.name, chromas });
    }
  }
  async loadChromas(cid, sid, summId) {
    var _a2;
    const chromasInv = await this.lcu.get(`/lol-champions/v1/inventories/${summId}/champions/${cid}/skins/${sid}/chromas`) || [];
    const ownedIds = chromasInv.filter(this.isOwned.bind(this)).map((c) => c.id);
    if (!ownedIds.length) return [];
    const detail = await getDetails(cid);
    const result = [];
    for (const skin of detail.skins || []) if (skin.id === sid) {
      for (const chroma of skin.chromas || []) if (ownedIds.includes(chroma.id)) {
        const label = chroma.name || chroma.displayName || `Chroma ${chroma.id}`;
        const color = ((_a2 = chroma.colors) == null ? void 0 : _a2[0]) || "N/A";
        result.push({ id: chroma.id, label, color });
      }
    }
    return result;
  }
  isOwned(o) {
    var _a2;
    return o.owned || o.isOwned || ((_a2 = o.ownership) == null ? void 0 : _a2.owned) || o.itemStatus === "OWNED";
  }
}
...
function createWindow() {
  ...
  ipcMain.handle("getState", () => picker.getState());
  picker.on("update", (st) => {
    win == null ? void 0 : win.webContents.send("stateUpdate", st);
  });
}
