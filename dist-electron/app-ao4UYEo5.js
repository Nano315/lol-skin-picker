...
class Body {
  ...
  async formData() {
    ...
    const { toFormData } = await import("./multipart-parser-B_2UXVS4.js");
...
class SkinsService extends EventEmitter {
  ...
  async rerollSkin() {
    ...
    const finalId = pick.chromas.length ? pick.chromas[Math.floor(Math.random() * pick.chromas.length)].id : pick.id;
  ...
  async refreshSkinsAndMaybeApply() {
    ...
    if (this.autoRollEnabled && this.currentChampion !== this.lastAppliedChampion && owned.length) {
      ...
      const finalId = picked.chromas.length ? picked.chromas[Math.floor(Math.random() * picked.chromas.length)].id : picked.id;
      const applied = await this.applySkin(finalId);
      if (!applied) return;
...
function registerSkinsIpc(svc, getWin) {
  ...
  ipcMain.handle("set-include-default", (_e, v) => svc.setIncludeDefault(!!v));
  ipcMain.handle("get-auto-roll", () => svc.getAutoRoll());
  ipcMain.handle("toggle-auto-roll", () => svc.toggleAutoRoll());
  ipcMain.handle("set-auto-roll", (_e, v) => svc.setAutoRoll(!!v));
