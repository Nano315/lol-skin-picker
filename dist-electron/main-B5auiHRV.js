...
class Body {
  ...
  async formData() {
    ...
    const { toFormData } = await import("./multipart-parser-D6tjEJt4.js");
...
class Picker extends EventEmitter {
  ...
  async tick() {
    ...
    if (!cid || cid === 0 || cid === this.state.championId) return;
    await loadNames();
  ...
  async loadSkins(cid) {
    ...
    if (!summ) return;
    const skinsInv = await this.lcu.get(`/lol-champions/v1/inventories/${summ.summonerId}/champions/${cid}/skins`) || [];
    const ownedSkins = skinsInv.filter(this.isOwned.bind(this));
    this.state.skins = [];
    for (const s of ownedSkins) {
      const chromas = await this.fetchChromas(cid, s.id, summ.summonerId);
      this.state.skins.push({ id: s.id, name: s.name, chromas });
    }
  }
  async fetchChromas(cid, sid, summId) {
    var _a2;
    const chromasInv = await this.lcu.get(`/lol-champions/v1/inventories/${summId}/champions/${cid}/skins/${sid}/chromas`) || [];
    if (!chromasInv.length) return [];
    ...
    for (const skin of detail.skins || []) if (skin.id === sid) {
      for (const chroma of skin.chromas || []) {
        ...
        chromaMap[chroma.id] = { label, color };
      }
    }
    return chromasInv.map((c) => {
      const m2 = chromaMap[c.id] || { label: `Chroma ${c.id}`, color: "N/A" };
      return { id: c.id, ...m2 };
    });
  }
  isOwned(o) {
