...
class Body {
  ...
  async formData() {
    ...
    const { toFormData } = await import("./multipart-parser-B79ntu2k.js");
...
async function createWindow() {
  ...
  const width = 900;
  const height = 645;
  let targetDisplay = displays.find((d) => d.id === settings.displayId) ?? screen.getDisplayNearestPoint(screen.getCursorScreenPoint()) ?? screen.getPrimaryDisplay();
...
function initAutoUpdate() {
  ...
  main$1.autoUpdater.on(
    "update-available",
    (info) => console.log("[Updater] available", info.version)
  );
  main$1.autoUpdater.on("update-not-available", () => console.log("[Updater] none"));
  main$1.autoUpdater.on(
    "download-progress",
    (p) => console.log(`[Updater] ${Math.round(p.percent)} %`)
  );
