...
class Body {
  ...
  async formData() {
    ...
    const { toFormData } = await import("./multipart-parser-BsaTAt7x.js");
...
const POLL = 2e3;
const LOCKFILE_GUESSES = [
  process.platform === "win32" ? "C:/Riot Games/League of Legends/lockfile" : "/Applications/League of Legends.app/Contents/LoL/lockfile"
];
class LCUConnector {
  constructor() {
  ...
  async connect() {
    ...
    const [port, password, protocol] = parts.length === 5 ? [parts[2], parts[3], parts[4]] : [parts[1], parts[2], parts[3]];
  ...
  findLockfile() {
    for (const guess of LOCKFILE_GUESSES)
...
class PickerCore extends EventEmitter {
  constructor() {
    super(...arguments);
    __publicField(this, "lcu", new LCUConnector());
  ...
  async tick() {
    ...
    if (phase === "ChampSelect") await this.updateDraftInfo();
  }
  // 1) updateDraftInfo : on re-essaie tant qu’on n’a pas de chromas
  async updateDraftInfo() {
    const cid = await this.lcu.get(
      "/lol-champ-select/v1/current-champion"
    );
    if (!cid || cid === 0) return;
    const firstTime = cid !== this.state.championId;
    if (!firstTime && this.state.skins.some((s) => s.chromas.length)) return;
    await loadNames();
  ...
  // 2) fetchChromas : fallback + nouveau test si la première requête est vide
  async fetchChromas(cid, sid, summId) {
    var _a2;
    let chromasInv = await this.lcu.get(
      `/lol-champions/v1/inventories/${summId}/champions/${cid}/skins/${sid}/chromas`
    ) || [];
    if (!chromasInv.length) {
      chromasInv = await this.lcu.get(
    ...
    for (const skin of detail.skins || [])
      if (skin.id === sid) {
        for (const c of skin.chromas || []) {
          const label = c.name || c.displayName || `Chroma ${c.id}`;
          const color = ((_a2 = c.colors) == null ? void 0 : _a2[0]) || "N/A";
          chromaMap[c.id] = { label, color };
        }
      }
    return chromasInv.map((c) => ({
      id: c.id,
      ...chromaMap[c.id] || { label: `Chroma ${c.id}`, color: "N/A" }
    }));
  }
  async loadSkins() {
    if (!this.state.championId) return;
    const summ = await this.lcu.get(
      "/lol-summoner/v1/current-summoner"
    );
    if (!summ) return;
    const skins = await this.lcu.get(
      `/lol-champions/v1/inventories/${summ.summonerId}/champions/${this.state.championId}/skins`
    );
    if (!skins) return;
