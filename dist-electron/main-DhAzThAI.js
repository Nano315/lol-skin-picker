...
class Body {
  ...
  async formData() {
    ...
    const { toFormData } = await import("./multipart-parser-DWOkCACF.js");
...
class LCU {
  ...
  async connect() {
    try {
      const parts = fs.readFileSync(LOCKFILE, "utf8").split(":");
  ...
  async get(ep) {
    ...
    try {
      const r2 = await fetch(this.base + ep, { headers: this.headers, timeout: 3e3 });
...
class Picker extends EventEmitter {
  ...
  async loadSkins(cid) {
    ...
    const owned = skins.filter(this.isOwned);
    const enriched = [];
    for (const s of owned) {
      const chromas = await this.loadChromas(cid, s.id, summ.summonerId);
      enriched.push({ id: s.id, name: s.name, chromas });
    }
    this.state.skins = enriched;
  }
  async loadChromas(cid, sid, summId) {
    var _a2;
    const chromas = await this.lcu.get(`/lol-champions/v1/inventories/${summId}/champions/${cid}/skins/${sid}/chromas`) || [];
    const ownedIds = chromas.filter(this.isOwned).map((c) => c.id);
