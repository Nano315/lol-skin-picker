...
const _LcuWatcher = class _LcuWatcher extends EventEmitter {
  ...
  readLockfile() {
    for (const p of _LcuWatcher.LOCKFILE_PATHS) {
      try {
...
__publicField(_LcuWatcher, "LOCKFILE_PATHS", [
...
class Body {
  ...
  async formData() {
    ...
    const { toFormData } = await import("./multipart-parser-CxxR8sVY.js");
...
class ChampionSkinWatcher extends EventEmitter {
  constructor() {
    ...
    __publicField(this, "champPoller", null);
  ...
  setCreds(creds) {
    this.creds = creds;
    this.obtainSummonerId();
  }
  /** Lance la détection (appelé quand on entre en ChampSelect) */
  start() {
    if (!this.creds || this.champPoller) return;
    this.tick();
    this.champPoller = setInterval(() => this.tick(), 2e3);
  }
  /** Stoppe quand on sort de ChampSelect ou qu’on perd la connexion */
  stop() {
    if (this.champPoller) clearInterval(this.champPoller);
    this.champPoller = null;
  ...
  async obtainSummonerId() {
    ...
    try {
      ...
      this.summonerId = data.summonerId ?? data.accountId ?? data.id;
    } catch {
    }
  }
  ...
  async updateSkins() {
    if (!this.creds || this.summonerId === null || !this.currentChampion)
      return;
    const { protocol, port, password } = this.creds;
    const url = `${protocol}://127.0.0.1:${port}/lol-champions/v1/inventories/${this.summonerId}/champions/${this.currentChampion}/skins`;
    const auth = Buffer.from(`riot:${password}`).toString("base64");
    try {
      const res = await fetch(url, {
        headers: { Authorization: `Basic ${auth}` }
      ...
      const data = await res.json();
      const owned = data.filter((s) => {
        var _a2;
        return ((_a2 = s.ownership) == null ? void 0 : _a2.owned) || s.isOwned || s.owned;
      }).map((s) => s.name || `Skin ${s.id}`);
      this.skins = owned;
      this.emit("skins", owned);
    } catch {
    }
  }
  /* ---------- typings pour l’autocomplétion ---------- */
  on(event, listener) {
    return super.on(event, listener);
  }
  emit(event, skins2) {
    return super.emit(event, skins2);
  }
}
