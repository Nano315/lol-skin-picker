...
class Body {
  ...
  async formData() {
    ...
    const { toFormData } = await import("./multipart-parser-qhwTYCgC.js");
...
(async () => {
  ...
  champMap = Object.fromEntries(
    (await (await fetch(url)).json()).map((c) => [c.id, c.name])
  );
})();
function findLockfile() {
  ...
  if (process.env.LOCALAPPDATA) {
    ...
    candidates.push(
      join(process.env.LOCALAPPDATA, "Riot Games", "Riot Client", "lockfile")
    );
  }
  ...
  for (const p of candidates) {
    if (existsSync(p)) {
      try {
        ...
        return { port, password, protocol, path: p };
      } catch {
      }
    }
  }
...
function authHeader(pass) {
  return "Basic " + Buffer.from(`riot:${pass}`).toString("base64");
}
async function fetchJSON(lock, api) {
  try {
    const r2 = await fetch(`https://127.0.0.1:${lock.port}${api}`, {
...
function push(update) {
  ...
  win == null ? void 0 : win.webContents.send("state-update", runtime);
...
async function startPolling() {
  ...
  setInterval(async () => {
    var _a2;
    if (!lock) lock = findLockfile();
    if (!lock) {
      push({ connected: false, lockfilePath: null });
      return;
    }
    push({ connected: true, lockfilePath: lock.path });
    ...
    if (phase === "ChampSelect") {
      ...
      if (!sess) return;
      const my = (_a2 = sess.myTeam) == null ? void 0 : _a2.find(
        (p) => p.cellId === sess.localPlayerCellId
      );
      const champId = my == null ? void 0 : my.championId;
      if (!champId) return;
      if (champId !== prevChamp) {
        prevChamp = champId;
        const champName = champMap[champId] ?? `ID ${champId}`;
        ...
        push({ champion: champName, skins: list });
      }
    } else {
      prevChamp = null;
      push({ champion: null, skins: [] });
    }
  }, 1e3);
}
