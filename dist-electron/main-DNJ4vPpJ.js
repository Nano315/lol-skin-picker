...
class _t {
  ...
  async formData() {
    ...
    const { toFormData: n } = await import("./multipart-parser-BATo9Kqu.js");
...
function Is() {
  ...
  sn = new Na(a), sn.setToolTip("LoL Skin Picker"), sn.on("double-click", () => B.isVisible() ? B.hide() : B.show()), process.env.VITE_DEV_SERVER_URL ? B.loadURL(process.env.VITE_DEV_SERVER_URL) : B.loadFile(tr(nr, "..", "dist", "index.html")), pn.start();
}
