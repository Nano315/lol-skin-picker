...
import require$$1$4, { BrowserWindow, ipcMain, shell, nativeImage, Tray, Menu, app, dialog, screen } from "electron";
...
class Body {
  ...
  async formData() {
    ...
    const { toFormData } = await import("./multipart-parser-BRft55_w.js");
...
class SkinsService extends EventEmitter {
  ...
  // Juste après un apply, on force une lecture immédiate pour réactivité
...
function setupTray(resolveAsset2, getWin) {
  const trayIcon = nativeImage.createFromPath(resolveAsset2("icon.ico"));
...
function resolveAsset(rel) {
  return app.isPackaged ? path$m.join(process.resourcesPath, rel) : path$m.join(process.cwd(), "public", rel);
}
