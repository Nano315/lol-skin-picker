...
import require$$1$4, { BrowserWindow, ipcMain, shell, nativeImage, Tray, app, Menu, dialog, screen } from "electron";
...
class Body {
  ...
  async formData() {
    ...
    const { toFormData } = await import("./multipart-parser-KMav4xYd.js");
...
class SkinsService extends EventEmitter {
  ...
  async rerollSkin() {
    ...
    const finalSkinId = (picked == null ? void 0 : picked.id) ?? pool[pickIndex(pool.length)].id;
  ...
  async rerollChroma() {
    ...
    const finalChromaId = (chroma == null ? void 0 : chroma.id) ?? skin.chromas[pickIndex(skin.chromas.length)].id;
  ...
  async refreshSkinsAndMaybeApply() {
    ...
    if (this.autoRollEnabled && this.currentChampion !== this.lastAppliedChampion && owned.length) {
      ...
      const pickedSkin = this.pickNextSkin(this.currentChampion, pool, this.selectedSkinId) ?? pool[pickIndex(pool.length)];
