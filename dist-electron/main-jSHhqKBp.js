...
class Body {
  ...
  async formData() {
    ...
    const { toFormData } = await import("./multipart-parser-CHIV8AWC.js");
...
class ChampionSkinWatcher extends EventEmitter {
  constructor() {
    ...
    // pour éviter de reroll plusieurs fois
  ...
  async refreshSkins() {
    ...
    if (this.currentChampion !== this.lastAppliedChampion && ownedSkins.length) {
      const pick = ownedSkins[Math.floor(Math.random() * ownedSkins.length)];
      await this.applySkin(pick.id);
