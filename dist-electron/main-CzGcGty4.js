...
class Body {
  ...
  async formData() {
    ...
    const { toFormData } = await import("./multipart-parser-gsRr7mva.js");
...
const POLL_MS = 2e3;
const LOCKFILE_PATH = process.platform === "win32" ? "C:/Riot Games/League of Legends/lockfile" : "/Applications/League of Legends.app/Contents/LoL/lockfile";
...
async function loadNames() {
  ...
  const res = await fetch(`${CD_BASE}/champion-summary.json`);
  const data = await res.json();
  for (const c of data) champName[c.id] = c.name;
}
const detailCache = {};
async function getDetails(cid) {
  if (detailCache[cid]) return detailCache[cid];
  const res = await fetch(`${CD_BASE}/champions/${cid}.json`);
  const json = await res.json();
  detailCache[cid] = json;
  return json;
}
class LCU {
  constructor() {
  ...
  async connect() {
    ...
    const [
      ...
      password,
      protocol
    ] = fs.readFileSync(LOCKFILE_PATH, "utf8").trim().split(":");
    this.base = `${protocol}://127.0.0.1:${portStr}`;
  ...
  async get(endpoint) {
    ...
    try {
      const r2 = await fetch(this.base + endpoint, { headers: this.headers });
...
class PickerCore extends EventEmitter {
  constructor() {
    super(...arguments);
  ...
  /* ---- boucle de polling ---- */
  async tick() {
    const connected = await this.lcu.connect();
    ...
    if (phase === "ChampSelect") await this.updateDraft();
  }
  /* ---- infos draft + chromas ---- */
  async updateDraft() {
    ...
    if (!cid || cid === 0) return;
    const needRefresh = cid !== this.state.championId || this.state.skins.every((s) => s.chromas.length === 0);
    if (!needRefresh) return;
    await loadNames();
    this.state.championId = cid;
    this.state.championName = champName[cid] || `#${cid}`;
    await this.populateSkinsAndChromas(cid);
    this.emit("update", this.state);
  }
  async populateSkinsAndChromas(cid) {
    ...
    if (!skinsInv) return;
    const ownedSkins = skinsInv.filter(
      (s) => {
        var _a2;
        return s.owned || s.isOwned || ((_a2 = s.ownership) == null ? void 0 : _a2.owned);
      }
    );
    const fullDetail = await getDetails(cid);
    const results = [];
    for (const s of ownedSkins) {
      const skinId = s.id;
      const chromas = await this.fetchChromas(
        cid,
        skinId,
        summ.summonerId,
        fullDetail
      );
      results.push({ id: skinId, name: s.name, chromas });
    }
    this.state.skins = results;
  }
  /* ----------------------------- */
  async fetchChromas(cid, sid, summId, championDetail) {
    var _a2;
    let inv = await this.lcu.get(
      `/lol-champions/v1/inventories/${summId}/champions/${cid}/skins/${sid}/chromas`
    ) || [];
    if (!inv.length) {
      inv = await this.lcu.get(
        `/lol-champions/v1/inventories/${summId}/skins/${sid}/chromas`
      ) || [];
    }
    if (!inv.length) return [];
    const chromaMeta = {};
    for (const skin of championDetail.skins || []) {
      if (skin.id !== sid) continue;
      for (const c of skin.chromas || []) {
        chromaMeta[c.id] = {
          label: c.name || c.displayName || `Chroma ${c.id}`,
          color: ((_a2 = c.colors) == null ? void 0 : _a2[0]) || "N/A"
        };
      }
    }
    return inv.map((c) => ({
      id: c.id,
      ...chromaMeta[c.id] || { label: `Chroma ${c.id}`, color: "N/A" }
    }));
  }
}
const picker = new PickerCore();
picker.start();
process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0";
