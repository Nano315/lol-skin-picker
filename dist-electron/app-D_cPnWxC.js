...
import require$$1$4, { BrowserWindow, ipcMain, shell, app, nativeImage, Tray, Menu, dialog, screen } from "electron";
import fs$j, { promises } from "node:fs";
import { EventEmitter } from "node:events";
import http from "node:http";
import https from "node:https";
import zlib from "node:zlib";
import Stream$1, { PassThrough, pipeline as pipeline$1 } from "node:stream";
import { Buffer as Buffer$1 } from "node:buffer";
import { types as types$1, deprecate, promisify } from "node:util";
import { format } from "node:url";
import { isIP } from "node:net";
import path$m, { join } from "node:path";
...
const _LcuWatcher = class _LcuWatcher extends EventEmitter {
  constructor() {
    super(...arguments);
    __publicField(this, "status", "disconnected");
    __publicField(this, "creds", null);
    __publicField(this, "timer", null);
    __publicField(this, "rawCache", "");
  }
  start(interval = 2e3) {
  ...
  readLockfile() {
    for (const p of _LcuWatcher.FILES) {
      try {
        return fs$j.readFileSync(p, "utf8");
      } catch {
      }
    }
    return null;
...
__publicField(_LcuWatcher, "FILES", [
  "C:\\Riot Games\\League of Legends\\lockfile",
  "C:\\Program Files\\Riot Games\\League of Legends\\lockfile"
]);
let LcuWatcher = _LcuWatcher;
      ...
      function ReadableStreamPipeTo(source, dest, preventClose, preventAbort, preventCancel, signal) {
        ...
        return newPromise((resolve, reject) => {
          ...
          function shutdownWithAction(action, originalIsError, originalError) {
            ...
            function doTheRest() {
              uponPromise(action(), () => finalize(originalIsError, originalError), (newError2) => finalize(true, newError2));
              return null;
            }
          }
...
var { toStringTag: t$2, iterator: i$1, hasInstance: h } = Symbol, r = Math.random, m = "append,set,get,getAll,delete,keys,values,entries,forEach,constructor".split(","), f = (a, b, c) => (a += "", /^(Blob|File)$/.test(b && b[t$2]) ? [(c = c !== void 0 ? c + "" : b[t$2] == "File" ? b.name : "blob", a), b.name !== c || b[t$2] == "blob" ? new File([b], c, b) : b] : [a, b + ""]), e = (c, f2) => (f2 ? c : c.replace(/\r?\n|\r/g, "\r\n")).replace(/\n/g, "%0A").replace(/\r/g, "%0D").replace(/"/g, "%22"), x = (n, a, e2) => {
  if (a.length < e2) {
...
const FormData = (_c = class {
  ...
  static [h](o) {
    return o && typeof o === "object" && o[t$2] === "FormData" && !m.some((m2) => typeof o[m2] != "function");
  }
  append(...a) {
...
class Body {
  ...
  async formData() {
    ...
    const { toFormData } = await import("./multipart-parser-Bn5II0gr.js");
...
class GameflowService extends EventEmitter {
  constructor() {
    super(...arguments);
    __publicField(this, "creds", null);
    __publicField(this, "timer", null);
    __publicField(this, "phase", "Unknown");
  }
  setCreds(creds) {
    this.creds = creds;
  ...
  async poll() {
    ...
    try {
      const txt = (await fetch(url, { headers: { Authorization: `Basic ${auth}` } }).then(
        (r2) => r2.text()
      )).replace(/"/g, "");
      if (txt !== this.phase) {
        this.phase = txt;
...
async function ensureAliasMap() {
  ...
  const data = await fetch(url).then((r2) => r2.json());
  data.forEach((ch) => aliasMap.set(ch.id, ch.alias));
}
function getChampionAlias(id) {
  return aliasMap.get(id) ?? "";
}
const SKIN_HISTORY = 3;
const CHROMA_HISTORY = 2;
function pickIndex$1(max) {
...
function weightedPick(items, weightFn) {
  ...
  if (sum <= 0) {
    return items[pickIndex$1(items.length)];
  }
  const r2 = randomInt(1e6) / 1e6;
  let acc = 0;
...
function chromaKey(championId, skinId) {
  return `${championId}:${skinId}`;
}
class SkinsService extends EventEmitter {
  constructor() {
    super(...arguments);
    __publicField(this, "creds", null);
    __publicField(this, "summonerId", null);
    __publicField(this, "poller", null);
    __publicField(this, "manualPoller", null);
    __publicField(this, "currentChampion", 0);
    __publicField(this, "lastAppliedChampion", 0);
    __publicField(this, "includeDefaultSkin", true);
    __publicField(this, "selectedSkinId", 0);
    __publicField(this, "selectedChromaId", 0);
    __publicField(this, "profileIconId", 0);
    __publicField(this, "autoRollEnabled", true);
    __publicField(this, "skins", []);
    // historiques récents
    __publicField(this, "skinHistory", /* @__PURE__ */ new Map());
    // championId -> [skinId,...]
    __publicField(this, "chromaHistory", /* @__PURE__ */ new Map());
    // "champ:skin" -> [chromaId,...]
    // compteurs "vus" (exploration équitable)
    __publicField(this, "skinSeenCounts", /* @__PURE__ */ new Map());
    // championId -> Map(skinId, count)
    __publicField(this, "chromaSeenCounts", /* @__PURE__ */ new Map());
  }
  // "champ:skin" -> Map(chromaId, count)
  setCreds(creds) {
    this.stop();
    this.creds = creds;
  ...
  async rerollSkin() {
    ...
    const pool = this.includeDefaultSkin ? this.skins : this.skins.filter((s) => s.id % 1e3 !== 0) || this.skins;
    const picked = this.pickNextSkin(
      this.currentChampion,
      pool,
      this.selectedSkinId
    );
    const finalSkinId = (picked == null ? void 0 : picked.id) ?? pool[pickIndex$1(pool.length)].id;
  ...
  async rerollChroma() {
    ...
    const finalChromaId = (chroma == null ? void 0 : chroma.id) ?? skin.chromas[pickIndex$1(skin.chromas.length)].id;
  ...
  async fetchSummonerId() {
    ...
    try {
      ...
      this.summonerId = r2.summonerId ?? r2.accountId ?? r2.id ?? null;
      this.profileIconId = r2.profileIconId ?? 0;
  ...
  emitSkinsIfChanged(next) {
    if (next.length === this.skins.length && next.every(
      (s, i) => {
        var _a2, _b2;
        return s.id === ((_a2 = this.skins[i]) == null ? void 0 : _a2.id) && s.chromas.length === ((_b2 = this.skins[i]) == null ? void 0 : _b2.chromas.length);
      }
    )) {
      return;
    }
  ...
  async refreshSkinsAndMaybeApply() {
    ...
    for (const s of allSkins.filter(
      (s2) => {
        var _a2;
        return ((_a2 = s2.ownership) == null ? void 0 : _a2.owned) || s2.isOwned || s2.owned;
      }
    )) {
      let chromaList = [];
      try {
        const chromas = await fetch(
          ...
          { headers }
        ).then((r2) => r2.status === 404 ? [] : r2.json());
        chromaList = chromas.filter((c) => {
          var _a2;
          return ((_a2 = c.ownership) == null ? void 0 : _a2.owned) || c.isOwned || c.owned;
    ...
    if (this.autoRollEnabled && this.currentChampion !== this.lastAppliedChampion && owned.length) {
      const pool = this.includeDefaultSkin ? owned : owned.filter((s) => s.id % 1e3 !== 0) || owned;
      const pickedSkin = this.pickNextSkin(this.currentChampion, pool, this.selectedSkinId) ?? pool[pickIndex$1(pool.length)];
  ...
  pickNextChroma(championId, skin, prevChromaId) {
    ...
    const seenCounts = this.chromaSeenCounts.get(key) ?? /* @__PURE__ */ new Map();
    ...
    if (filtered.length) candidates = filtered;
    if (!candidates.length) {
      candidates = chromas.length > 1 ? chromas.filter((c) => c.id !== prevChromaId) : chromas;
    }
  ...
  rememberSkin(championId, skinId) {
    const h2 = this.skinHistory.get(championId) ?? [];
    h2.push(skinId);
    if (h2.length > SKIN_HISTORY * 3) h2.shift();
    this.skinHistory.set(championId, h2);
    const counts = this.skinSeenCounts.get(championId) ?? /* @__PURE__ */ new Map();
    counts.set(skinId, (counts.get(skinId) ?? 0) + 1);
    this.skinSeenCounts.set(championId, counts);
  }
  rememberChroma(championId, skinId, chromaId) {
    const key = chromaKey(championId, skinId);
    const h2 = this.chromaHistory.get(key) ?? [];
    h2.push(chromaId);
    if (h2.length > CHROMA_HISTORY * 3) h2.shift();
    this.chromaHistory.set(key, h2);
    const counts = this.chromaSeenCounts.get(key) ?? /* @__PURE__ */ new Map();
  ...
  async updateManualSelection() {
    ...
    try {
      ...
      if (!selId || selId === this.selectedChromaId || selId === this.selectedSkinId)
        return;
      let skinId = selId;
      let chromaId = 0;
...
const WARD_HISTORY = 2;
function pickIndex(max) {
  if (max <= 1) return 0;
  return randomInt(0, max);
}
class WardService extends EventEmitter {
  constructor() {
    super(...arguments);
    __publicField(this, "creds", null);
    __publicField(this, "summonerId", null);
    __publicField(this, "ownedWardIds", []);
    __publicField(this, "recentWardIds", []);
    // mémoire glissante
    __publicField(this, "selectedWardId", 0);
    __publicField(this, "enabled", false);
  }
  // option "auto ward on lock"
  setCreds(creds) {
    this.creds = creds;
    this.summonerId = null;
    this.ownedWardIds = [];
  ...
  async fetchSummonerId() {
    ...
    try {
      ...
      this.summonerId = r2.summonerId ?? r2.id ?? r2.accountId ?? null;
    } catch {
      this.summonerId = null;
    }
  }
  async fetchOwnedWards() {
    ...
    try {
      ...
      this.ownedWardIds = list.filter((w) => {
        var _a2;
        return ((_a2 = w.ownership) == null ? void 0 : _a2.owned) || w.isOwned || w.owned;
      }).map((w) => w.id);
...
async function createMainWindow() {
  ...
  if (process.env.VITE_DEV_SERVER_URL) {
    ...
    const dist = path$m.join(process.cwd(), "dist");
    await win.loadFile(path$m.join(dist, "index.html"));
  }
  win.once("ready-to-show", () => win == null ? void 0 : win.show());
  return win;
}
function registerLcuIpc(lcu2) {
  ipcMain.handle("get-lcu-status", () => lcu2.status);
}
function registerGameflowIpc(gameflow2, getWin) {
  ipcMain.handle("get-gameflow-phase", () => gameflow2.phase);
  gameflow2.on("phase", (phase) => {
    const w = getWin();
    w == null ? void 0 : w.webContents.send("gameflow-phase", phase);
  });
}
function registerSkinsIpc(svc, getWin) {
  ...
  svc.on("skins", (list) => {
    var _a2;
    return (_a2 = getWin()) == null ? void 0 : _a2.webContents.send("owned-skins", list);
  });
  svc.on("selection", (sel) => {
    var _a2;
    return (_a2 = getWin()) == null ? void 0 : _a2.webContents.send("selection", sel);
  });
  svc.on("icon", (id) => {
    var _a2;
    return (_a2 = getWin()) == null ? void 0 : _a2.webContents.send("summoner-icon", id);
  });
}
...
async function loadSettings() {
  try {
    return JSON.parse(await promises.readFile(settingsPath, "utf-8"));
  } catch {
    return {};
  }
}
async function saveSettings(s) {
  try {
    await promises.writeFile(settingsPath, JSON.stringify(s, null, 2), "utf-8");
  } catch {
  }
}
function registerWardsIpc(wards2, getWin) {
  ipcMain.handle("get-auto-ward", () => wards2.getEnabled());
  ipcMain.handle("toggle-auto-ward", async () => {
    const next = !wards2.getEnabled();
    wards2.setEnabled(next);
    const s = await loadSettings();
    await saveSettings({ ...s, autoWard: next });
  });
  wards2.on("ward-list", (ids) => {
    var _a2;
    return (_a2 = getWin()) == null ? void 0 : _a2.webContents.send("ward-list", ids);
  });
  wards2.on(
    "ward-selection",
    (id) => {
      var _a2;
      return (_a2 = getWin()) == null ? void 0 : _a2.webContents.send("ward-selection", id);
    }
  );
}
function registerAllIpc(opts) {
...
if (util$2.debuglog)
  ...
  debug$3 = function() {
    var m2 = util$2.format.apply(util$2, arguments);
    m2 = "GFS4: " + m2.split(/\n/).join("\nGFS4: ");
    console.error(m2);
  };
...
function rmkids(p, options, cb) {
  ...
  options.readdir(p, (er, files) => {
    ...
    files.forEach((f2) => {
      rimraf$1(path$h.join(p, f2), options, (er2) => {
        if (errState) {
          return;
        }
...
function rmkidsSync(p, options) {
  assert(p);
  assert(options);
  options.readdirSync(p).forEach((f2) => rimrafSync(path$h.join(p, f2), options));
  if (isWindows) {
...
function parseUpdateInfo(rawData, channelFile, channelFileUrl) {
  ...
  try {
    ...
    throw (0, builder_util_runtime_1$e.newError)(`Cannot parse update info from ${channelFile} in the latest release artifacts (${channelFileUrl}): ${e2.stack || e2.message}, rawData: ${rawData}`, "ERR_UPDATER_INVALID_UPDATE_INFO");
  }
  return result;
}
  ...
  async getLatestVersion() {
    ...
    for (let attemptNumber = 0; ; attemptNumber++) {
      try {
        ...
        if (e2 instanceof builder_util_runtime_1$d.HttpError && e2.statusCode === 404) {
          throw (0, builder_util_runtime_1$d.newError)(`Cannot find channel "${channelFile}" update info: ${e2.stack || e2.message}`, "ERR_UPDATER_CHANNEL_FILE_NOT_FOUND");
...
class BitbucketProvider extends Provider_1$9.Provider {
  ...
  async getLatestVersion() {
    ...
    try {
      ...
      throw (0, builder_util_runtime_1$c.newError)(`Unable to find latest version on ${this.toString()}, please ensure release exists: ${e2.stack || e2.message}`, "ERR_UPDATER_LATEST_VERSION_NOT_FOUND");
    }
  }
  resolveFiles(updateInfo) {
...
class GitHubProvider extends BaseGitHubProvider {
  ...
  async getLatestVersion() {
    ...
    try {
      ...
      throw (0, builder_util_runtime_1$b.newError)(`Cannot parse releases feed: ${e2.stack || e2.message},
XML:
${feedXml}`, "ERR_UPDATER_INVALID_RELEASE_FEED");
    ...
    const fetchData = async (channelName) => {
      ...
      try {
        ...
        if (e2 instanceof builder_util_runtime_1$b.HttpError && e2.statusCode === 404) {
          throw (0, builder_util_runtime_1$b.newError)(`Cannot find ${channelFile} in the latest release artifacts (${channelFileUrl}): ${e2.stack || e2.message}`, "ERR_UPDATER_CHANNEL_FILE_NOT_FOUND");
        }
        throw e2;
      }
    };
  ...
  async getLatestTagName(cancellationToken) {
    ...
    try {
      ...
      throw (0, builder_util_runtime_1$b.newError)(`Unable to find latest version on GitHub (${url}), please ensure a production release exists: ${e2.stack || e2.message}`, "ERR_UPDATER_LATEST_VERSION_NOT_FOUND");
    }
  }
  get basePath() {
...
class KeygenProvider extends Provider_1$7.Provider {
  ...
  async getLatestVersion() {
    ...
    try {
      ...
      throw (0, builder_util_runtime_1$a.newError)(`Unable to find latest version on ${this.toString()}, please ensure release exists: ${e2.stack || e2.message}`, "ERR_UPDATER_LATEST_VERSION_NOT_FOUND");
    }
  }
  resolveFiles(updateInfo) {
...
class PrivateGitHubProvider extends GitHubProvider_1$1.BaseGitHubProvider {
  ...
  async getLatestVersion() {
    ...
    try {
      ...
      if (e2 instanceof builder_util_runtime_1$9.HttpError && e2.statusCode === 404) {
        throw (0, builder_util_runtime_1$9.newError)(`Cannot find ${channelFile} in the latest release artifacts (${url}): ${e2.stack || e2.message}`, "ERR_UPDATER_CHANNEL_FILE_NOT_FOUND");
      }
      throw e2;
    }
  ...
  async getLatestVersionInfo(cancellationToken) {
    ...
    try {
      ...
      throw (0, builder_util_runtime_1$9.newError)(`Unable to find latest version on GitHub (${url}), please ensure a production release exists: ${e2.stack || e2.message}`, "ERR_UPDATER_LATEST_VERSION_NOT_FOUND");
    }
  }
  get basePath() {
...
function doExecuteTasks(differentialDownloader, options, out2, resolve, reject) {
  ...
  const request = differentialDownloader.httpExecutor.createRequest(requestOptions, (response) => {
    ...
    const dicer = new DataSplitter_1$1.DataSplitter(out2, options, partIndexToTaskIndex, m2[1] || m2[2], partIndexToLength, resolve);
  ...
  downloadFile(tasks) {
    ...
    const closeFiles = () => {
      return Promise.all(fdList.map((openedFile) => {
        return (0, fs_extra_1$5.close)(openedFile.descriptor).catch((e2) => {
...
class AppUpdater extends events_1.EventEmitter {
  ...
  downloadUpdate(cancellationToken = new builder_util_runtime_1$4.CancellationToken()) {
    ...
    const errorHandler = (e2) => {
      if (!(e2 instanceof builder_util_runtime_1$4.CancellationError)) {
        try {
...
class NsisUpdater extends BaseUpdater_1.BaseUpdater {
  ...
  doInstall(options) {
    ...
    const callUsingElevation = () => {
      this.spawnLog(path.join(process.resourcesPath, "elevate.exe"), [installerPath].concat(args)).catch((e2) => this.dispatchError(e2));
    };
    ...
    this.spawnLog(installerPath, args).catch((e2) => {
...
function getTrayIconPath() {
  if (app.isPackaged) {
    const p = path$m.join(process.resourcesPath, "assets", "icon.ico");
    if (fs$j.existsSync(p)) return p;
    console.warn("[Tray] Not found:", p);
    return "";
  } else {
    const p = path$m.join(process.cwd(), "public", "icon.ico");
    if (fs$j.existsSync(p)) return p;
...
function setupTray(getWin) {
  ...
  const manualCheckForUpdates = () => {
    ...
    main$1.autoUpdater.checkForUpdates().catch((err) => {
      dialog.showErrorBox("Update error", (err == null ? void 0 : err.message) ?? String(err));
  ...
  refreshTrayMenu();
  setupTray.refresh = refreshTrayMenu;
}
function updaterHooks() {
  ...
  autoUpdater2.on("checking-for-update", () => {
    if (manualUpdateRequested) {
      ...
      dialog2.showMessageBox({ message: "Checking for updates…" });
    }
  });
  autoUpdater2.on("update-available", (info) => {
    if (manualUpdateRequested) {
      const { dialog: dialog2 } = require("electron");
  ...
  autoUpdater2.on("update-not-available", () => {
    if (manualUpdateRequested) {
      const { dialog: dialog2, app: app2 } = require("electron");
      dialog2.showMessageBox({
        type: "info",
        message: "You're up to date",
        detail: `Current version: ${app2.getVersion()}`
      });
      manualUpdateRequested = false;
    }
  });
  autoUpdater2.on("download-progress", (p) => {
    console.log(`[Updater] ${Math.round(p.percent)} %`);
  });
  autoUpdater2.on("update-downloaded", (info) => {
    const { dialog: dialog2 } = require("electron");
    if (manualUpdateRequested) {
      manualUpdateRequested = false;
      dialog2.showMessageBox({
        ...
        if (response === 0) autoUpdater2.quitAndInstall();
      });
    } else {
...
if (!app.isPackaged) {
  const devUserData = path$m.join(app.getPath("appData"), "lol-skin-picker-dev");
  app.setPath("userData", devUserData);
  app.commandLine.appendSwitch(
    "disk-cache-dir",
    path$m.join(devUserData, "Cache")
  );
...
function centerInDisplay(d, width, height) {
  const wa = d.workArea;
  const x2 = Math.floor(wa.x + (wa.width - width) / 2);
...
async function createWindowWithPrefs() {
  ...
  const width = 900, height = 645;
  const targetDisplay = displays.find((d) => d.id === settings.displayId) ?? screen.getDisplayNearestPoint(screen.getCursorScreenPoint()) ?? screen.getPrimaryDisplay();
  const w = await createMainWindow();
  const { x: x2, y } = centerInDisplay(targetDisplay, width, height);
  w.setBounds({ x: x2, y, width, height });
...
function wireDomainEvents() {
  lcu.on("status", (status, creds) => {
    var _a2, _b2, _c2;
    (_a2 = getMainWindow()) == null ? void 0 : _a2.webContents.send("lcu-status", status);
    if (status === "connected" && creds) {
      (_b2 = getMainWindow()) == null ? void 0 : _b2.show();
      ...
      gameflow.stop();
      skins.stop();
      (_c2 = getMainWindow()) == null ? void 0 : _c2.hide();
    }
  });
}
