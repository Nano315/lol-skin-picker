...
class Body {
  ...
  async formData() {
    ...
    const { toFormData } = await import("./multipart-parser-BONo-3Ut.js");
...
function readLockfile() {
  var _a2;
  const base = ((_a2 = process.env.LOCALAPPDATA) == null ? void 0 : _a2.replace(/\\/g, "/")) + "/Riot Games/League of Legends/lockfile";
...
async function fetchJSON(lock, path) {
  const res = await fetch(`https://127.0.0.1:${lock.port}${path}`, {
    headers: { Authorization: authHeader(lock.password) },
    agent
  }).catch(() => null);
...
async function loadChampionMap() {
  ...
  champMap = Object.fromEntries(arr.map((c) => [c.id, c.name]));
}
loadChampionMap();
async function startLCUPolling() {
  let lock = null;
  let summonerId = null;
  let prevChampionId = null;
  setInterval(async () => {
    var _a2;
    if (!lock) lock = readLockfile();
    if (!lock) {
      pushState({ connected: false });
      return;
    }
    pushState({ connected: true });
    const phase = await fetchJSON(lock, "/lol-gameflow/v1/gameflow-phase");
    pushState({ phase: phase ?? "Unknown" });
    if (!summonerId) {
      const me = await fetchJSON(lock, "/lol-summoner/v1/current-summoner");
      if (me == null ? void 0 : me.summonerId) summonerId = me.summonerId;
    }
    if (!summonerId) return;
    if (phase === "ChampSelect") {
      const sess = await fetchJSON(lock, "/lol-champ-select/v1/session");
      const myCell = (_a2 = sess == null ? void 0 : sess.myTeam) == null ? void 0 : _a2.find(
        (p) => p.cellId === sess.localPlayerCellId
      );
      const champId = (myCell == null ? void 0 : myCell.championId) ?? null;
      if (champId && champId !== prevChampionId) {
        prevChampionId = champId;
        ...
        const ownedSkins = (skins || []).filter(
          (s) => {
            var _a3;
            return s.isOwned || ((_a3 = s.ownership) == null ? void 0 : _a3.owned);
          }
        );
        const skinsWithChromas = [];
        for (const s of ownedSkins) {
          const chromas = await fetchJSON(
            lock,
            `/lol-champions/v1/inventories/${summonerId}/champions/${champId}/skins/${s.id}/chromas`
          );
          const ownedChromas = (chromas || []).filter(
            (c) => {
              var _a3;
              return c.isOwned || ((_a3 = c.ownership) == null ? void 0 : _a3.owned);
            }
          );
          skinsWithChromas.push({
            name: s.name ?? `Skin ${s.id}`,
            chromas: ownedChromas.map((c) => c.name ?? c.id)
          });
        }
        pushState({
          champion: champMap[champId] ?? `ID ${champId}`,
          skins: skinsWithChromas
        });
      }
    } else {
      prevChampionId = null;
      pushState({ champion: null, skins: [] });
    }
  }, 1e3);
}
function pushState(partial) {
  lastPayload = { ...lastPayload || {}, ...partial };
  if (win) win.webContents.send("state-update", lastPayload);
}
