...
class Body {
  ...
  async formData() {
    ...
    const { toFormData } = await import("./multipart-parser-BW8fEZgT.js");
...
class ChampionSkinWatcher extends EventEmitter {
  ...
  async refreshSkinsAndMaybeApply() {
    ...
    if (this.currentChampion !== this.lastAppliedChampion && owned.length) {
      ...
      const finalSkinId = pickedSkin.chromas.length ? pickedSkin.chromas[Math.floor(Math.random() * pickedSkin.chromas.length)].id : pickedSkin.id;
